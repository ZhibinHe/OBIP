...
parser.add_argument('--train_num_task', type=int, default=23, help='number of task of training')  #52
...
################## Define Dataloader ##################################
train_dataset, val_dataset, test_dataset, text_feature, test_text_feature = data_load_hcp_ya_intra_fmri(path, name, opt) #############################

dataset_filename = f'./model/stage1_intra_tr_hcp_ya_dataset.pt'
...
for epoch in range(0,stage1_epoch):
    ...
    model_filename = f'./model/stage1_intra_tr_hcp_ya_model_{epoch}.pth'
    dataset_filename =  f'./model/stage1_intra_tr_hcp_ya_dataset_{epoch}.pt'
    ...
    write_epoch_acc_to_file(epoch, val_acc, file_path='./model/stage1_intra_tr_hcp_ya_epoch_acc.txt')
...
df = pd.read_csv('./model/stage1_intra_tr_hcp_ya_epoch_acc.txt', sep=r'[,\s]+')
...
for i in range(all_epoch_acc.shape[0]):
    if i ==np.argmax(all_epoch_acc):
        continue
    model_filename = f'./model/stage1_intra_tr_hcp_ya_model_{all_epoch_epoch[i]}.pth'
    dataset_filename =  f'./model/stage1_intra_tr_hcp_ya_dataset_{all_epoch_epoch[i]}.pt'
...
model_filename = f'./model/stage1_intra_tr_hcp_ya_model_0.pth'
dataset_filename =  f'./model/stage1_intra_tr_hcp_ya_dataset_0.pt'
