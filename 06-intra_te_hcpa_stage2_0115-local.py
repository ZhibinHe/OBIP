...
parser.add_argument('--dataroot', type=str, default='/data/hzb/project/BodyDecoding/data/HCP_A_pcp/Schaefer/filt_noglobal', help='root directory of the dataset')
...
parser.add_argument('--phenoroot', type=str, default='/data/hzb/project/BodyDecoding_data/HCP_A/HCP_A_Phenotype')
parser.add_argument('--stage1_epochs', type=int, default=100, help='number of epochs of training')  #52
parser.add_argument('--datatoot_hcp', type=str, default='/data/hzb/project/BodyDecoding/data/HCP_A_pcp/Schaefer/filt_noglobal', help='root directory of the dataset')
parser.add_argument('--num_prediction', type=int, default=22, help='number prediction')
...
def test_acc1(loader):
    ...
    with torch.no_grad():
      ...
      for data in loader:
        data = data.to(device)
        data.y = data.y[:, opt.num_prediction:]
        all_datay = data.y

        if all_datay.shape[0] % opt.batchSize !=0:
            break
        else:
...
#################################
model1 = torch.load('./model/stage1_intra_tr_hcp_a_model_5.pth')
train_loader, val_loader,text_feature, text_feature_zero, fusion_feature_tr, fusion_feature_val, label_score_tr, label_score_val, regress_weight_tr, regress_weight_val = torch.load('./model/stage1_intra_tr_hcp_a_dataset_5.pt')
text_feature_all = text_feature


##across testing dataset
# index11 = [4,7,11,13,15,16,19,22 ,25,27,30 ,33]

#first
# train_dataset_hcp, val_dataset_hcp, test_dataset_hcp, _ ,  text_feature_hcp = data_load_hcp_a_intra_fmri(path, name, opt)
train_dataset_hcp, val_dataset_hcp, test_dataset_hcp, _ ,  text_feature_hcp = torch.load('./model/stage1_intra_tr_hcp_a_dataset.pt') #############################


val_dataset_hcp = DataLoader(val_dataset_hcp, batch_size=opt.batchSize, shuffle=True)
hcp_feature, hcp_label = test_acc1(val_dataset_hcp)
# torch.save([hcp_feature[:,index11,:], hcp_label[:,index11]], './model/stage2_intra_te_hcpa_feature_label.pt')
torch.save([hcp_feature, hcp_label], './model/stage2_intra_te_hcpa_feature_label.pt')

#second
hcp_feature, hcp_label = torch.load('./model/stage2_intra_te_hcpa_feature_label.pt')



##training dataset
# train_dataset, val_dataset, test_dataset, text_feature, test_text_feature = data_load_hcp_a_intra_fmri(path, name, opt) #############################
train_dataset, val_dataset, test_dataset, text_feature, test_text_feature = torch.load('./model/stage1_intra_tr_hcp_a_dataset.pt') #############################
...
def train2(epoch, train_loader_stage2):
    ...
    return loss_all / len(train_loader_stage2) #, s1_arr, s2_arr ,w1,w2        torch.nn.utils.clip_grad_norm_(parameters=model2.parameters(), max_norm=4.0)
...
for epoch in range(stage1_epoch, 300):
    since  = time.time()
    ...
    filename111 = f'./log/tr_hcp_a_te_hcp_a/label_sys_corr_epoch_{epoch}.npy'
    np.save(filename111, label_sys_corr)

    filename111 = f'./log/tr_hcp_a_te_hcp_a/sys_score_epoch_{epoch}.npy'
    np.save(filename111, sys_score)

    filename111 = f'./log/tr_hcp_a_te_hcp_a/label_score_epoch_{epoch}.npy'
