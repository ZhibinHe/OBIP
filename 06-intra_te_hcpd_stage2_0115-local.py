...
parser.add_argument('--dataroot', type=str, default='/data/hzb/project/BodyDecoding/data/HCP_D_pcp/Schaefer/filt_noglobal', help='root directory of the dataset')
...
parser.add_argument('--phenoroot', type=str, default='/data/hzb/project/BodyDecoding_data/HCP_D/HCP_D_Phenotype')
parser.add_argument('--stage1_epochs', type=int, default=100, help='number of epochs of training')  #52
parser.add_argument('--datatoot_hcp', type=str, default='/data/hzb/project/BodyDecoding/data/HCP_D_pcp/Schaefer/filt_noglobal', help='root directory of the dataset')
parser.add_argument('--num_prediction', type=int, default=21, help='number prediction')
...
#################################
model1 = torch.load('./model/stage1_intra_tr_hcp_d_model_2.pth')
train_loader, val_loader,text_feature, text_feature_zero, fusion_feature_tr, fusion_feature_val, label_score_tr, label_score_val, regress_weight_tr, regress_weight_val = torch.load('./model/stage1_intra_tr_hcp_d_dataset_2.pt')
text_feature_all = text_feature


##across testing dataset
# train_dataset_hcp, val_dataset_hcp, test_dataset_hcp, _ ,  text_feature_hcp = data_load_hcp_d_intra_fmri(path, name, opt)
train_dataset_hcp, val_dataset_hcp, test_dataset_hcp, _ ,  text_feature_hcp = torch.load('./model/stage1_intra_tr_hcp_d_dataset.pt') #############################
...
torch.save([hcp_feature, hcp_label], './model/stage2_intra_te_hcpd_feature_label.pt')

#second
hcp_feature, hcp_label = torch.load('./model/stage2_intra_te_hcpd_feature_label.pt')




##training dataset
# train_dataset, val_dataset, test_dataset, text_feature, test_text_feature = data_load_hcp_d_intra_fmri(path, name, opt) #############################
train_dataset, val_dataset, test_dataset, text_feature, test_text_feature = torch.load('./model/stage1_intra_tr_hcp_d_dataset.pt') #############################
...
for epoch in range(stage1_epoch, 300):
    ...
    filename111 = f'./log/tr_hcp_d_te_hcp_d/label_sys_corr_epoch_{epoch}.npy'
    np.save(filename111, label_sys_corr)

    filename111 = f'./log/tr_hcp_d_te_hcp_d/sys_score_epoch_{epoch}.npy'
    np.save(filename111, sys_score)

    filename111 = f'./log/tr_hcp_d_te_hcp_d/label_score_epoch_{epoch}.npy'
