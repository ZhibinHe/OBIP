...
#######################
parser.add_argument('--delete_percent', type=int, default=10)  #52
parser.add_argument('--test_dataset', type=str, default='HCP_YA')
parser.add_argument('--train_num_task', type=int, default=115, help='number of task of training')  #52
...
################## Define Dataloader ##################################
train_dataset, val_dataset, test_dataset, text_feature = data_load_abcd_fmri_delete_high(path, name, opt, device)
...
def test_acc1(loader):
    ...
    correct = np.nanmean(correct) #np.sum(correct)/pred_score.shape[1]
...
for epoch in range(0,stage1_epoch):
    ...
    #     tr_acc, tr_rmse, fusion_feature_tr, regress_weight_tr, label_score_tr, pred_score_tr = test_acc1(train_loader)
    #     val_acc, val_rmse, fusion_feature_val, regress_weight_val, label_score_val, pred_score_val = test_acc1(val_loader)
    #     model_filename = f'./model/delete_descrip_high/{opt.test_dataset}/stage1_across_tr_abcd_p{opt.delete_percent}_model_{epoch}.pth'
    #     dataset_filename =  f'./model/delete_descrip_high/{opt.test_dataset}/stage1_across_tr_abcd_p{opt.delete_percent}_dataset_{epoch}.pt'
    ...
    # file_path = f'./model/delete_descrip_high/{opt.test_dataset}/stage1_across_tr_abcd_p{opt.delete_percent}_epoch_acc.txt'
    # write_epoch_acc_to_file(epoch, val_acc, file_path)
...
#
# model_filename = f'./model/delete_descrip_high/{opt.test_dataset}/stage1_across_tr_abcd_p{opt.delete_percent}_model_0.pth'
# dataset_filename =  f'./model/delete_descrip_high/{opt.test_dataset}/stage1_across_tr_abcd_p{opt.delete_percent}_dataset_0.pt'
#
# if os.path.exists(model_filename):
#     os.remove(model_filename)
# if os.path.exists(dataset_filename):
