...
parser.add_argument('--num_prediction', type=int, default=71, help='number prediction')
parser.add_argument('--zero_num_prediction', type=int, default=44, help='number prediction')
...
#################################
model1 = torch.load('./model/stage1_intra_tr_abcd_model_8.pth')
train_loader, val_loader,text_feature, text_feature_zero, fusion_feature_tr, fusion_feature_val, label_score_tr, label_score_val, regress_weight_tr, regress_weight_val = torch.load('./model/stage1_intra_tr_abcd_dataset_8.pt')
...
train_dataset_hcp, val_dataset_hcp, test_dataset_hcp, _ ,  text_feature_hcp = torch.load('./model/stage1_intra_tr_abcd_dataset.pt') #############################
...
torch.save([hcp_feature, hcp_label], './model/stage2_intra_te_abcd_feature_label.pt')

#second
hcp_feature, hcp_label = torch.load('./model/stage2_intra_te_abcd_feature_label.pt')



##training dataset
# train_dataset, val_dataset, test_dataset, text_feature, test_text_feature = data_load_hcp_a_intra_fmri(path, name, opt) #############################
train_dataset, val_dataset, test_dataset, text_feature, test_text_feature = torch.load('./model/stage1_intra_tr_abcd_dataset.pt') #############################
...
for epoch in range(stage1_epoch, 300):
    ...
    filename111 = f'./log/tr_abcd_te_abcd/label_sys_corr_epoch_{epoch}.npy'
    np.save(filename111, label_sys_corr)

    filename111 = f'./log/tr_abcd_te_abcd/sys_score_epoch_{epoch}.npy'
    np.save(filename111, sys_score)

    filename111 = f'./log/tr_abcd_te_abcd/label_score_epoch_{epoch}.npy'
