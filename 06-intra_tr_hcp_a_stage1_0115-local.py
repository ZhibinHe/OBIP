...
parser.add_argument('--stage1_epochs', type=int, default=10, help='number of epochs of training')  #52
parser.add_argument('--train_num_task', type=int, default=22, help='number of task of training')  #52
...
dataset_filename = f'./model/stage1_intra_tr_hcp_a_dataset.pt'
torch.save( [train_dataset, val_dataset, test_dataset, text_feature, test_text_feature], dataset_filename)
...
text_feature_zero = test_text_feature #text_feature_all.repeat(opt.batchSize, 1, 1)
...
model1 = CombinedModel_sink_feature400_numtask(num_task=opt.train_num_task).to(device)     # ABCD 115
...
def train1(epoch):
    ...
    for data in train_loader:
        ...
        data.y = data.y[:,0:opt.train_num_task]
...
for epoch in range(0,stage1_epoch):
    ...
    model_filename = f'./model/stage1_intra_tr_hcp_a_model_{epoch}.pth'
    dataset_filename =  f'./model/stage1_intra_tr_hcp_a_dataset_{epoch}.pt'
    ...
    write_epoch_acc_to_file(epoch, val_acc, file_path='./model/stage1_intra_tr_hcp_a_epoch_acc.txt')
...
df = pd.read_csv('./model/stage1_intra_tr_hcp_a_epoch_acc.txt', sep=r'[,\s]+')
...
for i in range(all_epoch_acc.shape[0]):
    if i ==np.argmax(all_epoch_acc):
        continue
    model_filename = f'./model/stage1_intra_tr_hcp_a_model_{all_epoch_epoch[i]}.pth'
    dataset_filename =  f'./model/stage1_intra_tr_hcp_a_dataset_{all_epoch_epoch[i]}.pt'
...
model_filename = f'./model/stage1_intra_tr_hcp_a_model_0.pth'
dataset_filename =  f'./model/stage1_intra_tr_hcp_a_dataset_0.pt'
