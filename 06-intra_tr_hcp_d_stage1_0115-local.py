...
parser.add_argument('--train_num_task', type=int, default=21, help='number of task of training')  #52
...
dataset_filename = f'./model/stage1_intra_tr_hcp_d_dataset.pt'
...
for epoch in range(0,stage1_epoch):
    ...
    model_filename = f'./model/stage1_intra_tr_hcp_d_model_{epoch}.pth'
    dataset_filename =  f'./model/stage1_intra_tr_hcp_d_dataset_{epoch}.pt'
    ...
    write_epoch_acc_to_file(epoch, val_acc, file_path='./model/stage1_intra_tr_hcp_d_epoch_acc.txt')
...
df = pd.read_csv('./model/stage1_intra_tr_hcp_d_epoch_acc.txt', sep=r'[,\s]+')
...
for i in range(all_epoch_acc.shape[0]):
    if i ==np.argmax(all_epoch_acc):
        continue
    model_filename = f'./model/stage1_intra_tr_hcp_d_model_{all_epoch_epoch[i]}.pth'
    dataset_filename =  f'./model/stage1_intra_tr_hcp_d_dataset_{all_epoch_epoch[i]}.pt'
...
model_filename = f'./model/stage1_intra_tr_hcp_d_model_0.pth'
dataset_filename =  f'./model/stage1_intra_tr_hcp_d_dataset_0.pt'
