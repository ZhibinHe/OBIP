...
################## Define Dataloader ##################################
train_dataset, val_dataset, test_dataset, text_feature = data_load_hcp_a_fmri(path, name, opt)
...
for epoch in range(0,stage1_epoch):
    ...
    # model_filename = f'./model/stage1_across_tr_hcp_a_model_{epoch}.pth'
    # dataset_filename =  f'./model/stage1_across_tr_hcp_a_dataset_{epoch}.pt'
    ...
    # write_epoch_acc_to_file(epoch, val_acc, file_path='./model/stage1_across_tr_hcp_a_epoch_acc.txt')
...
df = pd.read_csv('./model/stage1_across_tr_hcp_a_epoch_acc.txt', sep=r'[,\s]+')
...
for i in range(all_epoch_acc.shape[0]):
    if i ==np.argmax(all_epoch_acc):
        continue
    model_filename = f'./model/stage1_across_tr_hcp_a_model_{all_epoch_epoch[i]}.pth'
    dataset_filename =  f'./model/stage1_across_tr_hcp_a_dataset_{all_epoch_epoch[i]}.pt'
...
model_filename = f'./model/stage1_across_tr_hcp_a_model_0.pth'
dataset_filename =  f'./model/stage1_across_tr_hcp_a_dataset_0.pt'
