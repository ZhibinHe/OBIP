...
from imports.HCPDataset import HCPfmriScoreDataset_sbjnum
...
text_feature = torch.load(os.path.join(opt.datadir+'/HCP_phenotype_feature_109_1030.pt'))



dataset = HCPfmriScoreDataset_sbjnum(path,name)
csvdata = pd.read_csv(os.path.join(opt.csvroot+'/S900_Release_Subjects_select_1030_tmp.csv'))
...
for i in range(traincsvdata['x_Subject'].shape[0]):
    select_score[i] = csvdata[traincsvdata['x_Subject'][i]]
...
# model = Network_regress_score_fMRI(fmri_indim = 333, fmri_outdim=1024, image_size=333, patch_size=333, num_classes=1024, dim=1024, depth=1, heads=16,mlp_dim=2048).to(device)

# image_feature_extractor = Network_regress_score_512(fmri_indim = 333, fmri_outdim=1024, image_size=333, patch_size=333, num_classes=1024, dim=1024, depth=1, heads=16,mlp_dim=2048).to(device)
# Phenotype_prediction = Network_phenotype_prediction(indim = 1024, outdim=1).to(device)
...
def train(epoch):
    ...
    for data in train_loader:
        ...
        # plt.imshow(dataset.data.x[0:333, :].detach().cpu().numpy())
        # plt.show()
        #
        ...
        # print(data.x.view(int(data.x.shape[0]/opt.nroi),opt.nroi, opt.nroi).shape)
...
def test_acc(loader):
    ...
    # pred_score = torch.tensor([0], device=device)
    #
    # label_score = torch.tensor([0], device=device)
    ...
    with torch.no_grad():
      ...
      for data in loader:
        ...
        # pred_score = torch.cat((pred_score, t1.data[:,:,0]), dim=0)
    ...
    # correct = np.corrcoef(pred_score[1:].detach().cpu().numpy().T, label_score[1:].detach().cpu().numpy().T)[0,1]
    ...
    # np.save('vit_sum_weight_best.npy', np.array(correct))  #29 epoch



    # ap = pred_score[1:].detach().cpu().numpy()
    # al = label_score[1:].detach().cpu().numpy()
    #
    # ap = (ap - np.min(ap)) / (np.max(ap) - np.min(ap))
    # al = (al - np.min(al)) / (np.max(al) - np.min(al))
    # epoch_rmse = np.sqrt(((ap-al) **2).mean())



    # np.save('pred_score', pred_score[1:, 0].detach().cpu().numpy().T)
    # np.save('label_score', label_score[1:].detach().cpu().numpy().T)


    return correct,  correct   #/ len(loader.dataset)
...
for epoch in range(0, num_epoch):
    ...
    tr_acc, tr_rmse = test_acc(train_loader)
    val_acc, val_rmse = test_acc(val_loader)
...
#
#     file.write(f"{head_str} {np.max(val_acc_list):.5f} {np.mean(val_acc_list):.5f} {np.std(val_acc_list):.5f}\n")
#
